...
import { updatePromptOutputForUser } from "@/lib/db";
...
export async function PATCH(
  ...
  const { data: prompt, error } = await updatePromptOutputForUser(
    supabase,
    parsedParams.data.id,
    parsedBody.data.output_prompt
  );

  if (error || !prompt) {
    logSupabaseError("prompts.update_output", error);
